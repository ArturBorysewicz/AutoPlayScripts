...
mouse_down_duration = 1.9  # Default mouse down duration
...
is_special_sequence_running = False  # New flag to track eating sequence
...
def update_mouse_down_duration(value=None):
    """Update the mouse down duration when the slider is moved or input is changed."""
    global mouse_down_duration
    try:
        if value is None:
            value = mouse_down_entry.get()
            mouse_down_duration = float(value)
            mouse_down_slider.set(mouse_down_duration)
        else:
            mouse_down_duration = float(value)
            mouse_down_entry.delete(0, "end")
            mouse_down_entry.insert(0, f"{mouse_down_duration:.2f}")
        mouse_down_label.config(text=f"Mouse Hold Duration: {mouse_down_duration:.2f}s")
    except ValueError:
        log_message("Invalid duration value. Please enter a number.")
...
def update_sequence_interval(value=None):
    """Update the special sequence interval when the slider is moved or input is changed."""
    global special_sequence_interval
    try:
        if value is None:
            value = sequence_interval_entry.get()
            special_sequence_interval = float(value)
            sequence_interval_slider.set(special_sequence_interval)
        else:
            special_sequence_interval = float(value)
            sequence_interval_entry.delete(0, "end")
            sequence_interval_entry.insert(0, f"{special_sequence_interval:.0f}")
        sequence_interval_label.config(text=f"Auto Eat Interval: {special_sequence_interval//60} min {special_sequence_interval%60} sec")
    except ValueError:
        log_message("Invalid interval value. Please enter a number.")
    ...
    while True:
        ...
        if (current_time - sequence_enabled_time >= special_sequence_interval and 
            ...
            try:
                ...
                time.sleep(4)
                # Food sequence
                log_message("Pressing 4 (food)...")
                pyautogui.keyDown('4')
                time.sleep(0.1)
                pyautogui.keyUp('4')
                time.sleep(4)  # Shorter delay for key press to register
                
                log_message("Clicking to eat...")
                pyautogui.click()
                time.sleep(8)  # Eating time
                
                # Drink sequence
                log_message("Pressing 5 (drink)...")
                pyautogui.keyDown('5')
                time.sleep(0.1)
                pyautogui.keyUp('5')
                time.sleep(4)
                
                log_message("Clicking to drink...")
                pyautogui.click()
                time.sleep(8)  # Drinking time
                ...
                log_message("Casting fishing rod...")
                pyautogui.mouseDown()
                time.sleep(mouse_down_duration)  # Use the configured hold duration
                pyautogui.mouseUp()
                time.sleep(1)  # Small delay after casting
...
def toggle_leftclick():
    global is_leftclick_enabled
    is_leftclick_enabled = leftclick_var.get()
    log_message(f"Left-click {'enabled' if is_leftclick_enabled else 'disabled'}")
...
mouse_down_label = ctk.CTkLabel(root, text=f"Default Cast Duration: {mouse_down_duration:.2f}s")
mouse_down_label.grid(row=3, column=0, padx=20, pady=0)

mouse_down_frame = ctk.CTkFrame(root)
mouse_down_frame.grid(row=4, column=0, padx=20, pady=10)

mouse_down_slider = ctk.CTkSlider(mouse_down_frame, from_=0.5, to=5.0, command=update_mouse_down_duration, width=200)
mouse_down_slider.set(mouse_down_duration)
mouse_down_slider.pack(side="left", padx=(0, 10))

mouse_down_entry = ctk.CTkEntry(mouse_down_frame, width=60)
mouse_down_entry.insert(0, f"{mouse_down_duration:.2f}")
mouse_down_entry.pack(side="left")
mouse_down_entry.bind("<Return>", lambda e: update_mouse_down_duration())

sequence_interval_label = ctk.CTkLabel(root, text=f"Default Auto Eat Interval: {special_sequence_interval//60} min {special_sequence_interval%60} sec")
sequence_interval_label.grid(row=5, column=0, padx=20, pady=0)

sequence_interval_frame = ctk.CTkFrame(root)
sequence_interval_frame.grid(row=6, column=0, padx=20, pady=10)

sequence_interval_slider = ctk.CTkSlider(sequence_interval_frame, from_=60, to=1800, command=update_sequence_interval, width=200)
sequence_interval_slider.set(special_sequence_interval)
sequence_interval_slider.pack(side="left", padx=(0, 10))

sequence_interval_entry = ctk.CTkEntry(sequence_interval_frame, width=60)
sequence_interval_entry.insert(0, f"{special_sequence_interval:.0f}")
sequence_interval_entry.pack(side="left")
sequence_interval_entry.bind("<Return>", lambda e: update_sequence_interval())

# Checkboxes
leftclick_var = ctk.BooleanVar(value=True)
leftclick_checkbox = ctk.CTkCheckBox(root, text="Enable Left-click", variable=leftclick_var, command=toggle_leftclick)
leftclick_checkbox.grid(row=7, column=0, padx=20, pady=10)

special_sequence_var = ctk.BooleanVar(value=False)
...
# Other UI elements
bind_label = ctk.CTkLabel(root, text="Binds: Rod (3), Food (4), Drink (5)")
